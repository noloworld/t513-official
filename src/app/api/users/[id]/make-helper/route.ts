...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    console.log('Iniciando processo de promoção a ajudante...');
    ...
    return NextResponse.json({ 
      message: 'Usuário promovido a ajudante com sucesso',
      user: updatedUser
    });
  } catch (error) {
    console.error('Erro ao promover usuário a ajudante:', error);
    return NextResponse.json(
